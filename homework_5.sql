#Создаю новую БД под vk c таблицами ДЗ начинается с 246 строчки...
CREATE DATABASE new_vk;
USE new_vk;
...
-- выбираю пользователя id = 11 (8-й мало сообщений получал)))

-- через переменную )))
SET @id_messages = (SELECT from_user_id FROM messages WHERE to_user_id = 11 GROUP BY from_user_id ORDER BY COUNT(from_user_id) DESC LIMIT 1);
SELECT @id_messages;
SELECT firstname, lastname from users WHERE id = @id_messages;
...
SELECT * FROM posts_likes GROUP BY post_id;

SELECT COUNT(*) AS 'likes cnt' FROM posts_likes WHERE user_id IN (
	SELECT user_id 
	FROM profiles
...
SELECT CASE (gender)
         WHEN 'm' THEN 'мужчины'
         WHEN 'f' THEN 'женщины'
    END AS 'Кто больше', COUNT(*)
FROM profiles WHERE user_id IN (
	SELECT user_id
	FROM posts_likes
) GROUP BY gender HAVING count(*) > 1 LIMIT 1;
